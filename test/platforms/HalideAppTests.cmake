...
# halide_add_*_generator_to_app (* may be ios, osx, pnacl, etc.)
#
...
function(halide_add_pnacl_app)
  ...
  set(NACL_SDK_ROOT "" CACHE PATH "Full path to the nacl_sdk directory containing pepper_* versions")
  ...
  # Compile the test code to a pnacl bc files
  set(test_outputs )
  foreach(test_source_path ${args_TEST_SOURCES})
    get_filename_component(test_source ${test_source_path} NAME_WE)
    ...
    set(test_outputs ${test_outputs} ${test_output})
  endforeach(test_source_path)

  # Add a build step to generate a .pexe target
  add_custom_target(${args_TARGET}
    WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
    COMMAND ${nacl_toolchain}/bin/pnacl-clang++ -o ${out_dir}/test_pnacl.pexe
      ${out_dir}/main.bc
      ${test_outputs}
      "\${HALIDE_PNACL_GENERATED_LIBRARIES}"
      -O2 ${nacl_linker_flags}
  ...
  # For most platforms we compile the app and all of the Halide generated
  # libraries separately, and then link them together based on
  # target_link_libraries() directives.
  # PNACL doesn't fit within this model and it isn't clear how to wire
  # pnacl-clang++ to the cmake compilation and link commands
  # As a workaround, we set an Xcode environment variable based on one of the
  # app target properties and use this to produce a list of .bc files to compile
  # together containing the Halide generated code.
  # TODO: Find a better cross platform way to wire up the pnacl tools.
...
### See the documentation for this function at the top of this file
function(halide_add_pnacl_generator_to_app)
  ...
  # Determine an output directory for this generator
  halide_generator_output_path(${args_GENERATOR_TARGET} out_dir)
  ...
  halide_add_generator_dependency(
    ...
    GENERATED_FUNCTION ${args_GENERATED_FUNCTION}
    GENERATOR_ARGS ${args_GENERATOR_ARGS}
    OUTPUT_LIB_VAR generator_library
    )

  # Add the generated .bc file to the main program compile line
  get_target_property(existing_libraries ${args_TARGET}
    XCODE_ATTRIBUTE_HALIDE_PNACL_GENERATED_LIBRARIES)
  set_target_properties(${args_TARGET} PROPERTIES
    XCODE_ATTRIBUTE_HALIDE_PNACL_GENERATED_LIBRARIES
